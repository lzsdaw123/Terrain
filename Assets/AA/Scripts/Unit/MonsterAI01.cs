...
public class MonsterAI01 : MonoBehaviour
{
    ...
    public float idlePercent = 30f;
    public float walkPercent = 50f;
    ...
    void Update()
    {
        ...
        else
        {
            ...
            {
                int r = (int)(Random.value * 100f); // 計算百分比用的參數

                if (r < idlePercent)  //若是idle動作
                {
                    moving = false;
                    speed = 0;
                }
                ...
                {
                    moving = true;
                    target = GetNavTarget();  //取得新的移動目標
                    agent.destination = target;  //設尋徑為新的目標

                    if (r < idlePercent + walkPercent) //走路
                    {
                        speed = 0.5f;
                    }
                    else
