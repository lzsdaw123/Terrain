...
public class MonsterAI04 : MonoBehaviour
{
    ...
    public float heightOfAttack = -1f; // 武器高度
    ...
    [SerializeField] private float TrackDistance; // 追蹤距離
    ...
    private void OnDrawGizmos()
    {
        ...
        if (attackLv1.displayGizmos)
        {
            ...
            GizmosExtension.DrawSector(transform.position + weaponHi, attackDistance, attackLv1.angle * 2, arcRotation);// 畫扇形
        }
    ...
    void Start()
    {
        ...
        agent.speed = agentSpeed; //要用動畫去移動,因此尋徑的速度要設為 0
    ...
    float GetXZAngle(Vector3 forward, Vector3 selfPosition, Vector3 targetPosition, bool signedNumber)
    {
    ...
    bool FindNearestPlayer(string[] PlayerTags, out Transform Player, out float Distance)
    {
        ...
        for (int i = 0; i < actors.Length; i++)
        {
            ...
            {
                // 若在視線角度內
                if (GetXZAngle(transform.forward, transform.position,actors[i].transform.position, false) < lookAngle)
                {
                    ...
                    if (NoObstacle(actors[i].transform)) // 若中間沒有障礙物
                    {
                       ...
                       actors[i].transform.position);
                                ...
                                tagObject.transform.position, false) < ArangeAngle)
                        {
                            ...
                            if (!attacking)
                            {
                                if (GetXZAngle(transform.forward, transform.position, tagObject.transform.position, false) < AttackAngle)
                                {
                                ...
                                else
                                {
                                    ...
                                    //transform.localRotation = Quaternion.Slerp(transform.localRotation, rotate, 8f * Time.smoothDeltaTime);
                                    ////print("轉向" + tagObject);
                                }
                            }                   
                        }
