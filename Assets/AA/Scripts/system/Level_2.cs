...
public class Level_2 : MonoBehaviour
{
    ...
    public void OnTriggerEnter(Collider other)  //開始第二關Boss戰
    {
        if (Level_A_Start)
        {
            if (other.gameObject.layer == LayerMask.NameToLayer("Actor"))
            {
                ...
                {
                    Level_A_Start = false;
                    Objects[0].transform.GetChild(0).GetComponent<Animator>().SetBool("Start",true); //觸發boss
                    LevelA_ = 2;
                    MissionTime = 0;
                }
            }
